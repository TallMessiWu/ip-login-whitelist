...
| 文件 | 用途 |
|---|---|
| `whitelist_manager.py` | CLI 全部功能 + Web 依赖的核心库，单文件 ~1220 行 |
...
| `requirements.txt` | `flask>=3.0.0`（paramiko 可选） |
...
- **认证**：Web 端 SHA-256+salt 密码哈希，session 鉴权，公开路径白名单（`/login`、`/guest`、`/apply`）
